"""
AFAC2026 赛题一 —— v4：证据分层 + 跨源确认（多源微观结构）

【对 v1→v3 的反思】
  Task2 占 60%，按 T+8 实盘回溯真值算加权 F1，但 v1/v2 是纯规则先验、
  v3 用「规则输出」当弱标签训 ML —— 自指循环，上限即规则本身，故 v3 退步。
  结论：规则（v2=0.5433）胜过自指 ML（v3=0.5311）。v4 不再赌 ML，
  而是保留可解释证据主干，喂入 100100 积分账号可取的**更高精度信号**：

  1. limit_list_d 连板/涨停/炸板 —— 连板≈游资铁证，比龙虎榜更干净
  2. cyq_perf 筹码/获利盘 —— winner_rate + 主力成本，正交意图信号
  3. 三源资金流(moneyflow/dc/ths)符号一致性 + 5日净额持续性 —— 稳健意图
  4. stk_limit 封板状态(封涨停≈买入) + bak_daily 攻击性/活跃度行为因子

【方法】Task2 = 高精度证据分层(连板/游资席位/量化席位/封板) → 跨源截面打分兜底；
        意图 = 封板 → 炸板出货 → 三源一致 → 5日持续 → T0。
        Task1 = 更丰富的判别特征 → 更高聚类区分度(轮廓/CH)，8 类唯一语义标签。

用法（项目根目录）：
  python shared/fetch_daily.py --start 20260608 --end <评测日> --out data/daily_hist.csv
  python v4/main_daily.py --target-date <评测日>
  python shared/make_submit.py --dir v4/out --zip v4/submit.zip
"""
import os
import re
import argparse
...
CLUSTER_FEATS = ['big_ratio', 'elg_ratio', 'lg_ratio', 'md_ratio', 'small_ratio',
                 'big_net_ratio', 'net_mf_ratio', 'turnover_rate', 'volume_ratio',
                 'pct_chg', 'amplitude', 'close_pos', 'hm_signal', 'inst_signal',
                 'll_is_up', 'll_limit_times', 'winner_rate', 'price_vs_cost',
                 'bk_attack', 'bk_activity', 'flow_agree']


def task1_clustering(tf):
    print("【2/4】Task1 交易模式聚类（21维判别特征 + 8类唯一语义标签）")
    ...
    prof_cols = (CLUSTER_FEATS + ['at_up_limit', 'll_is_zhaban', 'on_top_list', 'p_top_freq',
                 'flow_agree_dir'] +
...
def run(input_csv, out_dir, target_date=None, code_style='bare6'):
    ...
    print("\n下一步: python shared/make_submit.py --dir v4/out --zip v4/submit.zip")
