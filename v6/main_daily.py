"""
AFAC2026 赛题一 —— v6：capital_type 多日投票（在 v5=0.5906 基础上）

【只改 Task2 的 capital_type，其余全部沿用 v5】
  诊断发现 capital_type 是相对稳定的个股属性（跨日众数占比均值 0.76），
  但 v4/v5 每天独立判定，92 只软打分股在「量化↔散户↔游资」间抖动
  （相邻交易日预测 churn 高达 29.5%）。而赛题按 T+8 实盘回溯真值评分，
  真值本身在短期内是稳定的 —— 单日噪声纯属损失。

  v6 对【软证据】股做证据加权多日投票：
    - 当日硬证据(strong/mid)：直接采用当日判定（当日铁证最权威，不投票）
    - 当日软打分(soft)：对历史窗口按 证据权重×时间衰减 加权投票
      硬证据日权重 3.0 / 中 2.0 / 软 1.0；decay=0.8（越近权重越高）
  伪真值验证（以 51 只有硬证据股的历史众数为近似真值）：
    软打分股准确率 31.8% → 39.2%（+7.4pp）；全体 churn 29.5% → 15.8%。

  意图判断不变（v5 逐条规则），Task1 不变（v5 提纯聚类）。

用法（项目根目录）：
  python shared/fetch_daily.py --start 20260608 --end <评测日> --out data/daily_hist.csv
  python v6/main_daily.py --target-date <评测日>
  python shared/make_submit.py --dir v6/out --zip v6/submit.zip
"""
import os
import re
import argparse
...
# ── v6：capital_type 多日投票 ──────────────────────────────
EVIDENCE_WEIGHT = {'strong': 3.0, 'mid': 2.0, 'soft': 1.0}
VOTE_DECAY = 0.8   # 时间衰减：越近的交易日权重越高（伪真值验证最优）


def task2_recognition_voting(feat_hist, target_date):
    """对软证据股做证据加权多日投票；硬证据股仍用当日判定。

    - 逐个 <= target_date 的交易日重建当日截面（无未来函数），得到每股每日
      的 (label, evidence)；
    - target_date 当日为 strong/mid：直接采用（当日铁证最权威）；
    - target_date 当日为 soft：对全部历史窗口按 证据权重×decay^Δ天 加权投票。
    意图与聚类沿用 v5，不受影响。
    """
    print("【3/4】Task2 参与者识别（证据分层 + 跨源确认 + 多日投票）")
    days = sorted(d for d in feat_hist['transaction_date'].unique() if d <= target_date)
    # 每日预测缓存 {(stock, day): (label, evidence)}
    hist_pred = {}
    for d in days:
        tfd = build_day_frame(feat_hist, d)
        if tfd.empty:
            continue
        pr = tfd.apply(classify_capital, axis=1)
        for st, (lb, ev) in zip(tfd['stock_code'].values, pr.values):
            hist_pred[(st, d)] = (lb, ev)

    tf = build_day_frame(feat_hist, target_date)
    day_pred = tf.apply(classify_capital, axis=1)
    tf = tf.copy()
    tf['_today_type'] = [p[0] for p in day_pred.values]
    tf['_today_ev'] = [p[1] for p in day_pred.values]
    tgt_idx = days.index(target_date)

    def voted_type(stock, today_lb, today_ev):
        if today_ev in ('strong', 'mid'):
            return today_lb, False   # 当日铁证：不投票
        score = {}
        for d in days:
            key = (stock, d)
            if key not in hist_pred:
                continue
            lb, ev = hist_pred[key]
            delta = tgt_idx - days.index(d)          # 距目标日的天数（>=0）
            score[lb] = score.get(lb, 0.0) + EVIDENCE_WEIGHT[ev] * (VOTE_DECAY ** delta)
        if not score:
            return today_lb, False
        best = max(score.items(), key=lambda kv: kv[1])[0]
        return best, (best != today_lb)

    results, flips = [], 0
    for _, r in tf.iterrows():
        vt, flipped = voted_type(r['stock_code'], r['_today_type'], r['_today_ev'])
        results.append(vt)
        flips += flipped
    tf['capital_type'] = results
    tf['capital_intention'] = tf.apply(classify_intention, axis=1)

    ev_cnt = tf['_today_ev'].value_counts().to_dict()
    print(f"  证据层级: 硬证据={ev_cnt.get('strong',0)} 中={ev_cnt.get('mid',0)} 软打分={ev_cnt.get('soft',0)}")
    print(f"  多日投票翻转软打分: {flips} 只 (decay={VOTE_DECAY}, 全窗口)")
    res = tf[RESULT_COLUMNS]
    print(f"资金类型(投票后):\n{res['capital_type'].value_counts().to_string()}")
    print(f"交易意图:\n{res['capital_intention'].value_counts().to_string()}")
    return res
...
def run(input_csv, out_dir, target_date=None, code_style='bare6'):
    ...
    df_res = task2_recognition_voting(feat_hist, target_date)
    save_results(df_pat, df_res, out_dir, code_style)
    print("\n下一步: python shared/make_submit.py --dir v6/out --zip v6/submit.zip")
