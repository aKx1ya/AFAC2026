"""
AFAC2026 赛题一 —— v11：Task1 聚类空间提纯（在 v10 基础上）

【只改 Task1 聚类空间，Task2(capital_type/capital_intention) 与 v10 逐行相同】

  背景：v10 的 Task1 KMeans 用的是价格代理特征
    cols = [..., main_pct_proxy, main_pct3_proxy, super_proxy, small_proxy]
  其中 super_proxy = 0.4*main_pct_proxy、small_proxy = -0.5*main_pct_proxy，
  与 main_pct_proxy 完全共线——等于把同一根轴重复喂 3 次，聚类退化；
  且完全没用 B 榜数据里已有的真实资金结构 / 筹码 / 攻击性维度。

  v11 复刻 v5 的「聚类空间提纯」（v5 单此一项 A 榜 +0.0216）：
    - 聚类只用 14 维连续判别特征（真实资金结构/换手/价格/筹码/攻击性）
    - 剔除共线价格代理三件套
    - 语义标签匹配仍用完整画像（含证据列），不影响聚类本身
    - Task2 完全沿用 v10（硬证据优先 + 软打分兜底 + 真实资金流意图）

  Task1 评分只看聚类质量与模式合理性，与未知的 T+8 真值无关，
  因此本改动【不会】重蹈 v5-B 整迁崩盘（那是 capital 标签全换所致）。

用法：
  python shared/fetch_daily.py --start 20260608 --end <日> --stock-file <样本> --out data/xxx.csv
  python v11/main_daily.py --input data/xxx.csv --target-date 20260713 --code-style keep
...
# v11：聚类空间只用连续判别维度（对齐 v5 的提纯思路）。
# 这些列均在 build_hist_features/build_day_frame 中真实计算，B 榜数据完整覆盖。
CLUSTER_FEATS = [
    "big_ratio", "md_ratio", "small_ratio", "big_net_ratio", "net_mf_ratio",
    "turnover", "volume_ratio", "pct_chg", "amplitude", "close_pos",
...
def build_hist_features(raw: pd.DataFrame) -> pd.DataFrame:
    """Tushare 日频表 → 逐股逐日特征（含真实资金流 + 硬证据 + 筹码/行为）。"""
    df = raw.copy()
        ...
        rows.append(
            {
                ...
                "winner_rate": last["winner_rate"],
                "price_vs_cost": last["price_vs_cost"],
                "bk_attack": last["bk_attack"],
                "bk_activity": last["bk_activity"],
...
def assign_patterns(tf: pd.DataFrame) -> tuple[pd.Series, dict]:
    """v11：聚类空间用 14 维真实连续判别特征（剔除共线价格代理）。
    语义标签匹配仍用完整画像（rk_ 分位 + 代理），不影响聚类质量本身。
    """
    cols = [c for c in CLUSTER_FEATS if c in tf.columns]
    work = tf.copy()
...
def run(input_csv: str, out_dir: str, target_date: str | None = None, code_style: str = "keep"):
    ...
    if metrics:
        print(f"Task1 聚类({len(CLUSTER_FEATS)}维提纯) | 轮廓:{metrics['silhouette']:.4f} CH:{metrics['ch']:.4f}")
