"""
AFAC2026 赛题一 —— v7：Task1 每股定制化解释（在 v5=0.5906 基础上）

【只改 pattern_explanation，其余全部沿用 v5】
  赛题 Task1 明确「通过标签的合理性和可解释性打分」。v2~v6 的
  pattern_explanation 是【同一 pattern 下所有股票共用一句模板】——
  可解释性弱。v7 把它改成【逐股定制、引用该股当日真实数字】的说明：
    涨跌幅/封板状态 → 换手量能 → 资金结构与净流向 → 硬证据(游资/机构/连板)
    → 获利盘/成本 → 该模式的研判结论
  例：「涨停封板(2连板)，换手8.5%、放量(量比2.1)，大单主导、资金净流入
      (+3.2%)，游资明细2家；研判为游资短线抢筹拉升」

  关键：pattern_type 与聚类过程与 v5 逐行完全一致（不动聚类、不动标签分配、
  不动 Task2），因此 Task1 的聚类质量指标与 Task2 输出均与 v5 相同，
  本次分差可 100% 归因于「解释文本可解释性」。

用法（项目根目录）：
  python shared/fetch_daily.py --start 20260608 --end <评测日> --out data/daily_hist.csv
  python v7/main_daily.py --target-date <评测日>
  python shared/make_submit.py --dir v7/out --zip v7/submit.zip
"""
import os
import re
import argparse
...
def build_explanation(r, pattern):
    """为单只股票当日生成引用真实数字的解释文本。

    结构：价格/封板 → 换手量能 → 资金结构与净流向 → 硬证据 → 获利盘/成本 → 研判结论。
    仅陈述当日事实 + 该 pattern 的研判语，末尾附标签名，确保可解释且不空泛。
    """
    parts = []

    # 1) 价格行为 / 封板状态
    pct = float(r.get('pct_chg', 0) or 0)
    if r.get('at_up_limit', 0) > 0:
        lt = int(r.get('ll_limit_times', 0) or 0)
        parts.append(f"收盘涨停({lt}连板)" if lt >= 2 else "收盘涨停")
    elif r.get('at_dn_limit', 0) > 0:
        parts.append("收盘跌停")
    elif r.get('ll_is_zhaban', 0) > 0:
        parts.append(f"盘中触板回落({_fmt_pct(pct)})")
    else:
        trend = "上涨" if pct > 0.5 else ("下跌" if pct < -0.5 else "平收")
        parts.append(f"{trend}{_fmt_pct(pct)}")

    # 2) 换手 / 量能
    turn = float(r.get('turnover_rate', 0) or 0)
    vr = float(r.get('volume_ratio', 0) or 0)
    amp = float(r.get('amplitude', 0) or 0)
    vol_desc = "放量" if vr >= 1.5 else ("缩量" if vr < 0.8 else "量能平稳")
    parts.append(f"换手{_fmt_pct(turn)}、{vol_desc}(量比{vr:.1f})、振幅{_fmt_pct(amp)}")

    # 3) 资金结构 + 净流向
    big = float(r.get('big_ratio', 0) or 0)
    small = float(r.get('small_ratio', 0) or 0)
    net = float(r.get('net_mf_ratio', 0) or 0)
    if big >= 0.35:
        struct = "大单主导"
    elif small >= 0.55:
        struct = "小单为主"
        ...
        struct = "中单活跃"
    if net > 0.02:
        flow = f"资金净流入({_fmt_pct(net*100)})"
    elif net < -0.02:
        flow = f"资金净流出({_fmt_pct(net*100)})"
    else:
        flow = "净额基本平衡"
    parts.append(f"{struct}、{flow}")

    # 4) 硬证据（游资/机构/量化席位）
    ev = []
    if r.get('hm_has_quant', 0) > 0:
        ev.append("量化游资席位")
    if r.get('inst_seats', 0) > 0:
        ev.append("机构专用席位")
    hm = int(r.get('hm_count', 0) or 0)
    if hm > 0 and r.get('hm_has_quant', 0) <= 0:
        ev.append(f"游资明细{hm}家")
    if r.get('on_top_list', 0) > 0 and not ev:
        ev.append("登龙虎榜")
    if ev:
        parts.append("、".join(ev))

    # 5) 获利盘 / 成本偏离（有筹码数据时）
    wr = r.get('winner_rate', None)
    pvc = r.get('price_vs_cost', None)
    try:
        wr = float(wr)
        if wr >= 70:
            parts.append(f"获利盘高({_fmt_pct(wr,0)})")
        elif wr <= 15:
            parts.append(f"套牢盘重(获利仅{_fmt_pct(wr,0)})")
    except (ValueError, TypeError):
        pass

    body = "，".join(parts)
    verdict = PATTERN_DESC.get(pattern, pattern).split('，')[-1]
    return f"{body}；研判为{verdict}"
...
def task1_clustering(tf):
    ...
    # v7：逐股定制解释（引用该股当日真实数字），而非同 pattern 共用模板
    dp['pattern_explanation'] = [
        build_explanation(row, pat)
        for (_, row), pat in zip(tf.iterrows(), dp['pattern_type'])
    ]
...
def run(input_csv, out_dir, target_date=None, code_style='bare6'):
    ...
    print("\n下一步: python shared/make_submit.py --dir v7/out --zip v7/submit.zip")
