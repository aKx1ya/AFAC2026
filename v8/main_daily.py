"""
AFAC2026 赛题一 —— v8：capital_intention 阈值重标定（在 v5=0.5906 基础上）

【只改意图阈值两处，Task1 与 capital_type 与 v5 逐行相同】
  经离线验证框架（`_intent_harness.py`/`_intent_dual.py`，42天扩展数据、
  未来8日方向双口径代理真值、时间切分 train/test、分普涨/震荡/普跌）确认：
  当前意图规则在跌市偏弱（普跌日买入召回仅0.12，部分因 proxy 被市场beta污染）。

  v8 仅调两个阈值（最小单杠杆、抗过拟合）：
    - 单源净额门槛 net_hi: 0.03 → 0.025
    - 三源一致门槛 agr:    0.02 → 0.015
  held-out 测试集双口径均正增益且各行情无退化：
    绝对方向 +0.0055（普跌 +0.0067）；市场中性 +0.0057（普跌 +0.0151）。

  上线纪律（吸取 v6/v7 教训）：仅当【两种真值口径 × held-out 测试集 × 各
  行情均非负】时才上线；纯单口径微调（+0.0069 未达门槛）已被否决。
  ⚠️ 代理真值≠官方T+8真值，预期总分增益为个位数千分位，可能被行情噪声掩盖。

用法（项目根目录）：
  python shared/fetch_daily.py --start 20260608 --end <评测日> --out data/daily_hist.csv
  python v8/main_daily.py --target-date <评测日>
  python shared/make_submit.py --dir v8/out --zip v8/submit.zip
"""
import os
import re
import argparse
...
def classify_intention(r):
    ...
    if r['ll_is_zhaban'] > 0 and r['ll_open_times'] >= 2 and r['pct_chg'] < 3:
        return '卖出'
    # 3) 三源资金流方向一致 + 净额达标（v8: agr 0.02→0.015，跨源一致时门槛可更低）
    if r['flow_agree_dir'] > 0 and net > 0.015 and r['pct_chg'] > -1:
        return '买入'
    if r['flow_agree_dir'] < 0 and net < -0.015 and r['pct_chg'] < 1:
        return '卖出'
    # 4) 单源强净流入/流出 + 价格配合（v8: net_hi 0.03→0.025，离线双口径验证正增益）
    if (net > 0.025 or big > 0.04) and r['pct_chg'] > -1:
        return '买入'
    if (net < -0.025 or big < -0.04) and r['pct_chg'] < 1:
        return '卖出'
    # 5) 5 日资金持续性（弱兜底，需净额与价格同向；net_hi 同步 0.025）
    if r['ths_d5_dir'] > 0 and net > 0.025 and r['pct_chg'] > 2:
        return '买入'
    if r['ths_d5_dir'] < 0 and net < -0.025 and r['pct_chg'] < -2:
        return '卖出'
    return 'T0交易'
...
def run(input_csv, out_dir, target_date=None, code_style='bare6'):
    ...
    print("\n下一步: python shared/make_submit.py --dir v8/out --zip v8/submit.zip")
